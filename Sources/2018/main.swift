...
func executeChallenge(day: String, part: String, isSolved: String?) {
    ...
    let fileURL = URL(fileURLWithPath: "Inputs/2018/Day\(dayNumber)Input.txt")
    ...
    case "p1":
        ...
        if (isSolved != nil) {
            ...
            task.arguments = ["Scripts/submit.sh", "2018", dayNumber, "1", result]
            task.launch()
            task.waitUntilExit()
        }
        print("=========================")
    case "p2":
        ...
        if (isSolved != nil) {
            let task = Process()
            task.launchPath = "/bin/bash"
            task.arguments = ["Scripts/submit.sh", "2018", dayNumber, "2", result]
            task.launch()
            task.waitUntilExit()
        }
        print("=========================")
